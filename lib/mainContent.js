...
const calculateFileD = require('./calculateFileD.js');
const calculateFileF = require('./calculateFileF.js');
...
const buildMainContent = (fullStaticPath, pathname)=>{
    ...
    try{
        ...
        return `<div class="alert alert-danger">Internal Server Error 1</div>`;
    }
    ...
    if(pathname === '/'){
        items = items.filter(element => !['project_files', 'ace-build-master'].includes(element));
    }
    ...
    items.forEach(item => {
        ...
        if(itemDetails.stats.isDirectory()){
            ...
            [itemDetails.size, itemDetails.sizeBytes] = calculateFileD(itemFullStaticPath);
        }else if (itemDetails.stats.isFile()){
            itemDetails.icon = '<ion-icon name="document"></ion-icon>';
            
            [itemDetails.size, itemDetails.sizeBytes] = calculateFileF(itemDetails.stats);
        }
